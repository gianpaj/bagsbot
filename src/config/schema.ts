...
export const creatorFilterSchema = z.object({
  requireVerifiedSocial: z.boolean().describe('Whether to require verified social accounts'),
  minFollowerCount: z
    .number()
    .int()
  ...
  checkPreviousLaunches: z.boolean().describe('Whether to check creator previous token launches'),
});
...
export const technicalFilterSchema = z.object({
  requireCompleteMetadata: z.boolean().describe('Whether to require complete metadata'),
  requireDescription: z.boolean().describe('Whether to require a description'),
  requireSocialLinks: z.boolean().describe('Whether to require at least one social link'),
  validateImageUrl: z.boolean().describe('Whether to validate image URL accessibility'),
});
...
export const socialFilterSchema = z.object({
  checkTwitterMentions: z.boolean().describe('Whether to check for Twitter/X mentions'),
  checkTelegramGroup: z.boolean().describe('Whether to check for Telegram group presence'),
  minCommunitySize: z
    .number()
    .int()
...
export const scoringConfigSchema = z
  ...
  .refine((config) => config.minScoreToAlert <= config.minScoreForHighConfidence, {
...
export const exitConfigSchema = z.object({
  ...
  autoSellEnabled: z.boolean().describe('Whether automatic selling is enabled'),
});
...
export const uiConfigSchema = z.object({
  ...
  soundEnabled: z.boolean().describe('Whether to play sounds for alerts'),
  headless: z.boolean().optional().default(true).describe('Run in headless mode (no terminal UI)'),
});
...
export const launchSourceConfigSchema = z.object({
  ...
  scenarioName: z
    ...
    .describe('Scenario preset to use when launch source is scenario'),
  scenarioIntervalMs: z
    .number()
    .int()
    .min(100, 'Scenario interval must be at least 100ms')
    .describe('Delay between scenario launch events in milliseconds'),
  disableTrading: z.boolean().describe('Whether to disable trade execution in scenario mode'),
});
...
export const botConfigSchema = z.object({
  ...
  solanaRpcUrl: z.url('Solana RPC URL must be a valid URL').describe('Solana RPC endpoint URL'),
  walletPath: z
    .string()
  ...
  launchSource: launchSourceConfigSchema,
  ui: uiConfigSchema,
});
